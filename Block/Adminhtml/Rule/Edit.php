 ...
 */
namespace Lof\CouponCode\Block\Adminhtml\Rule;
...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    protected function _construct()
    {
        ...
        if ($this->_isAllowedAction('Lof_CouponCode::rule_save')) { 
        ...
        if ($this->_isAllowedAction('Lof_CouponCode::rule_delete')) {
            $this->buttonList->update('delete', 'label', __('Delete Email'));
        } else {
    ...
    {
        if ($this->_coreRegistry->registry('lofcouponcode_rule')->getId()) {
            return __("Edit Form '%1'", $this->escapeHtml($this->_coreRegistry->registry('lofcouponcode_rule')->getTitle()));
        } else {
            return __('New Form');
        }
    }
