...
namespace Lof\CouponCode\Model\Log;

use Lof\CouponCode\Model\ResourceModel\Log\CollectionFactory;
...
class DataProvider extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    public function getData()
    {
        ...
        $data = $this->dataPersistor->get('lof_couponcode_log');
        
        if (!empty($data)) {
            $model = $this->collection->getNewEmptyItem();
            $model->setData($data);
            $this->loadedData[$model->getId()] = $model->getData();
            $this->dataPersistor->clear('lof_couponcode_log');
        }
